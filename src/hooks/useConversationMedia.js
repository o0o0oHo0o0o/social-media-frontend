  ...
  const fetchPhotos = useCallback(async (page = 0) => {
    ...
    try {
      const response = await fetch(
        withBase(`/api/chat/${conversationId}/photos?page=${page}&size=20`),
        { credentials: 'include' }
      );
      if (!response.ok) {
...
export const useConversationFiles = (conversationId) => {
  ...
  const fetchFiles = useCallback(async (page = 0) => {
    ...
    try {
      const response = await fetch(
        withBase(`/api/chat/${conversationId}/files?page=${page}&size=10`),
        { credentials: 'include' }
      );
...
export const useConversationMembers = (conversationId) => {
  ...
  const fetchMembers = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        withBase(`/api/chat/${conversationId}/members/details`),
        { credentials: 'include' }
      );
